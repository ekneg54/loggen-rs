...
- `src/main.rs` — CLI entrypoint. `clap::Parser` + `Subcommand` derive, `prog <subcommand> help` workaround at `main.rs:63-71`.
...
- `examples/` — 4 YAML config fixtures.
...
- Template validation **panics** on unknown variables at `Generator::new()` (`generator.rs:260`).
